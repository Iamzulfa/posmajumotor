          ...
          builder: (context, setDialogState) => AlertDialog(
            ...
            content: SizedBox(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  final brand = brands[index];
                  final brandId = 'brand_${brand.id}';
                  final isSelected = tempSelectedBrands.contains(brandId);
                  ...
                  return CheckboxListTile(
                    ...
                    onChanged: (bool? value) {
                      setDialogState(() {
                        if (value == true) {
                          tempSelectedBrands.add(brandId);
                        } else {
                          tempSelectedBrands.remove(brandId);
                        }
                      });
                    },
                  );
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () {
                  // Apply the temporary state to actual state
                  setState(() {
                    // Remove old brand filters
                    for (final brandId in _selectedBrands) {
                      _filterManager.removeFilter(brandId);
                    }

                    // Add new brand filters
                    _selectedBrands = List.from(tempSelectedBrands);
                    for (final brandId in _selectedBrands) {
                      final brand = brands.firstWhere(
                        (b) => 'brand_${b.id}' == brandId,
                      );
