...
class PdfGenerator {
  /// Generate PDF untuk profit/loss report (auto-detect daily or monthly)
  static Future<void> generateProfitLossReport({
    required ProfitLossReport report,
    required String businessName,
    DateTime? date, // If provided, generate daily report; otherwise monthly
  }) async {
    if (date != null) {
      await generateDailyProfitLossReport(
        report: report,
        date: date,
        businessName: businessName,
      );
    } else {
      await generateMonthlyProfitLossReport(
        report: report,
        businessName: businessName,
      );
    }
  ...
  /// Generate PDF untuk daily profit/loss report
  static Future<void> generateDailyProfitLossReport({
    required ProfitLossReport report,
    ...
    try {
      ...
      await Printing.sharePdf(
        ...
        filename:
            'Laporan_Harian_${DateFormat('dd_MM_yyyy', 'id_ID').format(date)}.pdf',
      );

      AppLogger.info('Daily PDF generated successfully');
    } catch (e) {
      AppLogger.error('Error generating daily PDF', e);
      rethrow;
    }
  }

  /// Generate PDF untuk monthly profit/loss report
  static Future<void> generateMonthlyProfitLossReport({
    required ProfitLossReport report,
    required String businessName,
  }) async {
    try {
      ...
      await Printing.sharePdf(
        bytes: await pdf.save(),
        filename:
            'Laporan_Bulanan_${report.month.toString().padLeft(2, '0')}_${report.year}.pdf',
      );
